...
const DayPickerIndicator: React.SFC = () => {
    return (
...
export class DateFilter extends React.Component<Props, any> {
    refs: {
        [key: string]: (Element);
    ...
    componentDidMount() {
        ...
        if (this.props.filter.mode === 'between') {
            findDOMNode<HTMLElement>(this.refs[this.state.activeInputField]).focus();
        } else {
    ...
    selectMode(e) {
        ...
        if (mode === 'between') {
            setTimeout(() => { findDOMNode<HTMLElement>(this.refs[this.state.activeInputField]).focus(); }, 200);
        } else {
            findDOMNode<HTMLElement>(this.refs.input).focus();
        }
    }
