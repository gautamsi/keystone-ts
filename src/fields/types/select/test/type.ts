...
export const initList = function (List) {
    List.add({
        ...
        numeric: {
            type: SelectType, numeric: true, options: [
                { value: 1, label: 'one' },
                { value: 2, label: 'two' },
                { value: 3, label: 'three' },
            ]
...
export const testFieldType = function (List) {
    ...
    describe('updateItem', function () {
        it('should update top level fields', function (done) {
            ...
            List.fields.select.updateItem(testItem, {
                ...
                demand(testItem.select).be.equal('one');
                done();
            });
        });
        ...
        it('should update nested fields', function (done) {
            ...
            List.fields['nested.select'].updateItem(testItem, {
                ...
                demand(testItem.nested.select).be.equal('one');
                done();
            });
        });
    ...
    it('should format values with the label of the option', function () {
        ...
        demand(List.fields.select.format(testItem)).be.equal('One');
    });
    ...
    it('should pluck custom properties from the selected option', function () {
        ...
        demand(testItem._.extraProps.pluck('custom')).be.equal('2');
    });
    ...
    it('should have the label in nameLabel', function () {
        ...
        demand(testItem.extraPropsLabel).be.equal('Two');
    });
    ...
    it('should return a blank string when formatting an undefined value', function () {
        const testItem = new List.model();
        demand(List.fields.select.format(testItem)).be.equal('');
    });
