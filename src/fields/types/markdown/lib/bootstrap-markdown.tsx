...
let Markdown = function (element, options) {
	// Class Properties
...
, __buildButtons: function(buttonsArray, container) {
		...
		for (i = 0; i < buttonsArray.length; i++) {
			...
			let y, btnGroups = buttonsArray[i];
...
, __localize: function(string) {
		let messages = $.fn.markdown.messages,
...
, hidePreview: function() {
		...
		let container = this.$editor.find('div[data-provider="markdown-preview"]');
...
, findSelection: function(chunk) {
	let content = this.getContent(), startChunkPosition;

	if (startChunkPosition = content.indexOf(chunk), startChunkPosition >= 0 && chunk.length > 0) {
		let oldSelection = this.getSelection(), selection;
...
, enableButtons: function(name) {
		let buttons = this.__parseButtonNameParam(name),
			that = this;
...
let analyzeMarkdown = function(e) {
	let blurred = false,
			el,
