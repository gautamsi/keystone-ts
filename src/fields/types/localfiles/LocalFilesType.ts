 ...
 * @api public
 */
export function localfiles (list, path, options) {
...
localfiles.prototype.addToSchema = function (schema) {
	...
	const exists = function (item, element_id) {
	...
	const reset = function (item, element_id) {
		if (typeof element_id === 'undefined') {
			item.set(field.path, []);
		} else {
			const values = item.get(field.path);
			// allow implicit type coercion to compare string IDs with MongoID objects
			const value = values.find(function (val) { return val._id == element_id; }); // eslint-disable-line eqeqeq
			if (typeof value !== 'undefined') {
...
localfiles.prototype.uploadFiles = function (item, files, update, callback) {
	...
	async.map(files, function (file, processedFile) {
		...
		const doMove = function (doneMove) {
